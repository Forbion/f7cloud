require('../assets/index-BrcnDXgp.css');
"use strict";
...
const initChunkWorkspace = async function(destinationFile = void 0, retries = 5, isPublic = false) {
  let chunksWorkspace;
  if (isPublic) {
    chunksWorkspace = `${router.getBaseUrl()}/public.php/dav/uploads/${_public.getSharingToken()}`;
  } else {
    chunksWorkspace = router.generateRemoteUrl(`dav/uploads/${auth.getCurrentUser()?.uid}`);
  }
...
class Eta extends typescriptEventTarget.TypedEventTarget {
  /** Bytes done */
  _done = 0;
  /** Total bytes to do */
  _total = 0;
   ...
   */
  get speedReadable() {
    return this._speed > 0 ? `${files.formatFileSize(this._speed, true)}∕s` : "";
  }
}
...
class Uploader {
  ...
  // Global upload queue
  _uploadQueue = [];
  _jobQueue = new PQueue__default.default({
    // Maximum number of concurrent uploads
    // @ts-expect-error TS2339 Object has no defined properties
    concurrency: capabilities.getCapabilities().files?.chunked_upload?.max_parallel_count ?? 5
  });
  _queueSize = 0;
  _queueProgress = 0;
  _queueStatus = 0;
  ...
  batchUpload(destination, files$1, callback) {
    ...
    return new PCancelable__default.default(async (resolve, reject, onCancel) => {
      ...
      await rootFolder.addChildren(files$1);
      ...
      try {
        ...
        if (axios.isCancel(error) || error instanceof UploadCancelledError) {
  ...
  upload(destination, fileHandle, root, retries = 5) {
    ...
    const promise = new PCancelable__default.default(async (resolve, reject, onCancel) => {
      ...
      const supportsPublicChunking = capabilities.getCapabilities().dav?.public_shares_chunking ?? false;
...
function showInvalidFilenameDialog(error) {
  const InvalidFilenameDialog = Vue.defineAsyncComponent(() => Promise.resolve().then(() => require("./InvalidFilenameDialog-BOBIqCQq.cjs")));
  const { promise, reject, resolve } = Promise.withResolvers();
  dialogs.spawnDialog(
    InvalidFilenameDialog,
    {
      error,
      validateFilename: files.validateFilename
    },
    (...rest) => {
...
function uploadConflictHandler(contentsCallback) {
  return async (nodes, path2) => {
    try {
      const content = await contentsCallback(path2).catch(() => []);
      ...
      for (const file of nodes) {
        try {
          files.validateFilename(file.name);
...
const _sfc_main = Vue.defineComponent({
  ...
  beforeMount() {
    ...
    useHotKey_js.useHotKey("u", this.onKeyDown, {
      stop: true,
      prevent: true,
      shift: true
    });
    useHotKey_js.useHotKey("Escape", this.onKeyDown, {
      stop: true,
      prevent: true
    });
  ...
  methods: {
    ...
    async onPick() {
      ...
      try {
        await this.uploadManager.batchUpload("", files2, uploadConflictHandler(this.getContent));
      } catch (error) {
...
async function openConflictPicker(dirname, conflicts, content, options) {
  const ConflictPicker = Vue.defineAsyncComponent(() => Promise.resolve().then(() => require("./ConflictPicker-BkU7oyxg.cjs")));
...
exports.t = t;
exports.upload = upload;
exports.uploadConflictHandler = uploadConflictHandler;
