require('../assets/ConflictPicker-CpKPg_rh.css');
"use strict";
...
const NcCheckboxRadioSwitch = require("@nextcloud/vue/dist/Components/NcCheckboxRadioSwitch.js");
...
const NcDateTime = require("@nextcloud/vue/dist/Components/NcDateTime.js");
...
var __component__$5 = /* @__PURE__ */ index.normalizeComponent(
  _sfc_main$5,
  _sfc_render$5,
...
var __component__$4 = /* @__PURE__ */ index.normalizeComponent(
  _sfc_main$4,
  _sfc_render$4,
...
var __component__$3 = /* @__PURE__ */ index.normalizeComponent(
  _sfc_main$3,
  _sfc_render$3,
...
var __component__$2 = /* @__PURE__ */ index.normalizeComponent(
  _sfc_main$2,
  _sfc_render$2,
...
const _sfc_main$1 = Vue.defineComponent({
  ...
  components: {
    FileSvg,
    FolderSvg,
    NcCheckboxRadioSwitch: NcCheckboxRadioSwitch__default.default,
    NcDateTime: NcDateTime__default.default
  },
  props: {
    incoming: {
      type: [File, Object],
  ...
  watch: {
    ...
    incoming: {
      ...
      async handler() {
        ...
        } else if (index.isFileSystemFileEntry(this.incoming)) {
  ...
  methods: {
    ...
    size(node) {
      if (node.size) {
        return files.formatFileSize(node.size, true);
      }
      return index.t("Unknown size");
    },
    previewUrl(node) {
      ...
      if (node.type === files.FileType.Folder) {
        return null;
      }
      try {
    ...
    isFolder(node) {
      if (index.isFileSystemEntry(node)) {
        return node.isDirectory;
      }
      node = node;
      return node.type === files.FileType.Folder || node.type === "httpd/unix-directory";
    },
...
var __component__$1 = /* @__PURE__ */ index.normalizeComponent(
  _sfc_main$1,
  _sfc_render$1,
...
const _sfc_main = Vue.defineComponent({
  ...
  setup() {
    return {
      blockedTitle: index.t("You need to select at least one version of each file to continue.")
    };
  },
  data() {
    return {
  ...
  computed: {
    name() {
      if (this?.dirname?.trim?.() !== "") {
        return index.n("{count} file conflict in {dirname}", "{count} file conflicts in {dirname}", this.conflicts.length, {
          count: this.conflicts.length,
          dirname: this.dirname
        });
      }
      return index.n("{count} file conflict", "{count} files conflict", this.conflicts.length, { count: this.conflicts.length });
    },
    skipButtonLabel() {
      if (this.conflicts.length === 1) {
        return index.t("Skip this file");
      }
      return index.n("Skip {count} file", "Skip {count} files", this.conflicts.length, { count: this.conflicts.length });
    },
  ...
  mounted() {
    this.files = this.conflicts.map((conflict) => {
      const name = conflict instanceof File || index.isFileSystemEntry(conflict) ? conflict.name : conflict.basename;
    ...
    index.logger.debug("ConflictPicker initialised", { files: this.files, conflicts: this.conflicts, content: this.content });
  },
  methods: {
    onCancel(error) {
      this.opened = false;
    ...
    onSkip() {
      index.logger.debug("Conflict skipped. Ignoring all conflicting files");
      this.opened = false;
      this.$emit("submit", {
        selected: [],
        renamed: []
    ...
    onSubmit() {
      if (!this.isEnoughSelected) {
        ...
        dialogs.showError(this.blockedTitle);
        return;
      }
      ...
      const toRename = this.newSelected.filter((node) => {
        const name = node instanceof File || index.isFileSystemEntry(node) ? node.name : node.basename;
        return selectedOldNames.includes(name);
      });
      if (toRename.length > 0) {
        toRename.forEach((file) => {
          const name = file instanceof File || index.isFileSystemEntry(file) ? file.name : file.basename;
          const newName = files.getUniqueName(name, directoryContent);
          if (file instanceof File || index.isFileSystemEntry(file)) {
      ...
      index.logger.debug("Conflict resolved", { selected, renamed });
      this.opened = false;
      this.$emit("submit", {
        selected,
        renamed
    ...
    onSelectAllNew(selected) {
      if (selected) {
        index.logger.debug("Selected all new files");
        this.newSelected = this.conflicts;
      } else {
        index.logger.debug("Cleared new selection");
        this.newSelected = [];
      }
    },
    onSelectAllOld(selected) {
      if (selected) {
        index.logger.debug("Selected all existing files");
        this.oldSelected = this.files;
      } else {
        index.logger.debug("Cleared old selection");
        this.oldSelected = [];
      }
    },
    t: index.t
...
exports.default = ConflictPicker;
