...
function translate(app, text, vars, number, options) {
  ...
  const optSanitize = allOptions.sanitize ? DOMPurify__default.default.sanitize : identity;
  const optEscape = allOptions.escape ? escapeHTML__default.default : identity;
  ...
  const _build = (text2, vars2, number2) => {
    return text2.replace(/%n/g, "" + number2).replace(/{([^{}]*)}/g, (match, key) => {
      ...
      if (isValidReplacement(replacement)) {
        ...
        const escape = replacement.escape !== false ? escapeHTML__default.default : identity;
...
function loadTranslations(appName, callback) {
  if (hasAppTranslations(appName) || locale.getLocale() === "en") {
    return Promise.resolve().then(callback);
  }
  const url = router.generateFilePath(appName, "l10n", locale.getLocale() + ".json");
...
exports.getCanonicalLocale = locale.getCanonicalLocale;
exports.getLanguage = locale.getLanguage;
exports.getLocale = locale.getLocale;
exports.isRTL = locale.isRTL;
