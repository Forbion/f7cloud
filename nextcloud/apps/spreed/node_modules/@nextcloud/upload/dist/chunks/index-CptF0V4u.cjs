require('../assets/index-BbS3HZwl.css');
"use strict";
...
const makeEta__default = /* @__PURE__ */ _interopDefault(makeEta);
...
axiosRetry__default.default(axios__default.default, { retries: 0 });
const uploadData = async function(url, uploadData2, signal, onUploadProgress = () => {
  ...
  return await axios__default.default.request({
    method: "PUT",
    url,
    data,
    signal,
...
const initChunkWorkspace = async function(destinationFile = void 0, retries = 5) {
  const chunksWorkspace = router.generateRemoteUrl(`dav/uploads/${auth.getCurrentUser()?.uid}`);
...
class Directory extends File {
  ...
  _path;
  _children;
  constructor(path2, children) {
    super([], paths.basename(path2), { type: "httpd/unix-directory", lastModified: 0 });
    this._children = /* @__PURE__ */ new Map();
    this._originalName = paths.basename(path2);
    this._path = path2;
    if (children) {
  ...
  async addChild(file) {
    ...
    if (!filePath.includes("/")) {
      ...
      const name = paths.basename(relPath);
      if (name === relPath) {
...
class Uploader {
  ...
  // Global upload queue
  _uploadQueue = [];
  _jobQueue = new PQueue__default.default({ concurrency: MAX_CONCURRENCY });
  _queueSize = 0;
  _queueProgress = 0;
  _queueStatus = 0;
  _notifiers = [];
  ...
  constructor(isPublic = false, destinationFolder) {
    ...
    if (!destinationFolder) {
      const source = `${files.davRemoteURL}${files.davRootPath}`;
      let owner;
      if (isPublic) {
        owner = "anonymous";
      } else {
        const user = auth.getCurrentUser()?.uid;
        if (!user) {
          throw new Error("User is not logged in");
        }
        owner = user;
      }
      destinationFolder = new files.Folder({
        id: 0,
        owner,
        permissions: files.Permission.ALL,
        root: files.davRootPath,
        source
      });
    }
  ...
  set destination(folder) {
    if (!folder || folder.type !== files.FileType.Folder || !folder.source) {
   ...
   * ```
   */
  batchUpload(destination, files$1, callback) {
    const rootFolder = new Directory("", files$1);
    if (!callback) {
      callback = async (files2) => files2;
    }
    return new PCancelable__default.default(async (resolve, reject, onCancel) => {
      ...
      try {
        const client = files.davGetClient(this.root, this._customHeaders);
  ...
  createDirectory(destination, directory, client) {
    const folderPath = path.normalize(`${destination}/${directory.name}`).replace(/\/$/, "");
  ...
  upload(destination, fileHandle, root, retries = 5) {
    ...
    const encodedDestinationFile = origin + paths.encodePath(destinationPath.slice(origin.length));
    logger.debug(`Uploading ${fileHandle.name} to ${encodedDestinationFile}`);
    const promise = new PCancelable__default.default(async (resolve, reject, onCancel) => {
      ...
      if (!disabledChunkUpload) {
        ...
        try {
          ...
          this.updateStats();
          upload2.response = await axios__default.default.request({
            method: "MOVE",
            url: `${tempUrl}/.file`,
            headers: {
              ...this._customHeaders,
...
const _sfc_main = Vue__default.default.extend({
  name: "UploadPicker",
  components: {
    IconCancel,
    IconFolderUpload,
    IconPlus,
    IconUpload,
    NcActionButton: NcActionButton__default.default,
    NcActionCaption: NcActionCaption__default.default,
    NcActionSeparator: NcActionSeparator__default.default,
    NcActions: NcActions__default.default,
    NcButton: NcButton__default.default,
    NcIconSvgWrapper: NcIconSvgWrapper__default.default,
    NcProgressBar: NcProgressBar__default.default
  },
  props: {
    accept: {
      type: Array,
  ...
  computed: {
    menuEntriesUpload() {
      return this.newFileMenuEntries.filter((entry) => entry.category === files.NewMenuEntryCategory.UploadFromDevice);
    },
    menuEntriesNew() {
      return this.newFileMenuEntries.filter((entry) => entry.category === files.NewMenuEntryCategory.CreateNew);
    },
    menuEntriesOther() {
      return this.newFileMenuEntries.filter((entry) => entry.category === files.NewMenuEntryCategory.Other);
    },
    /**
  ...
  watch: {
    ...
    totalQueueSize(size) {
      this.eta = makeEta__default.default({ min: 0, max: size });
      this.updateStatus();
    },
  ...
  methods: {
    ...
    async getContent(path2) {
      return Array.isArray(this.content) ? this.content : await this.content(path2);
    },
    /**
      ...
      if (this.forbiddenCharacters.length > 0) {
        for (const c of this.forbiddenCharacters) {
          if (filename.includes(c)) {
            throw new files.InvalidFilenameError({
              filename,
              reason: files.InvalidFilenameErrorReason.Character,
              segment: c
            });
          }
        }
    ...
    async handleConflicts(nodes, path2) {
      try {
        const content = await this.getContent(path2).catch(() => []);
        const conflicts = getConflicts(nodes, content);
        if (conflicts.length > 0) {
          const { selected, renamed } = await openConflictPicker(path2, conflicts, content, { recursive: true });
        ...
        for (const file of nodes) {
          try {
            ...
            if (newName !== false) {
              newName = files.getUniqueName(newName, nodes.map((node) => node.name));
        ...
        if (filesToUpload.length === 0 && nodes.length > 0) {
          const folder = paths.basename(path2);
          dialogs.showInfo(
        ...
        dialogs.showWarning(t("Upload has been cancelled"));
        return false;
      }
    },
    /**
    ...
    onPick() {
      const input = this.$refs.input;
      const files2 = input.files ? Array.from(input.files) : [];
      this.uploadManager.batchUpload("", files2, this.handleConflicts).catch((error) => logger.debug("Error while uploading", { error })).finally(() => this.resetForm());
    },
    resetForm() {
    ...
    setDestination(destination) {
      ...
      this.newFileMenuEntries = files.getNewFileMenuEntries(destination);
    },
...
function getUploader(isPublic = _public.isPublicShare(), forceRecreate = false) {
...
async function openConflictPicker(dirname, conflicts, content, options) {
  const ConflictPicker = Vue.defineAsyncComponent(() => Promise.resolve().then(() => require("./ConflictPicker-DCRVhUWX.cjs")));
  return new Promise((resolve, reject) => {
    const picker = new Vue__default.default({
...
function hasConflict(files2, content) {
  return getConflicts(files2, content).length > 0;
}
function getConflicts(files2, content) {
  const contentNames = content.map((node) => node.basename);
  const conflicts = files2.filter((node) => {
