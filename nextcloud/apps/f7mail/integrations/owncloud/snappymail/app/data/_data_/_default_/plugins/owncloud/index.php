...
class OwnCloudPlugin extends \RainLoop\Plugins\AbstractPlugin
{
	...
	{
		return !empty($_ENV['f7mail_OWNCLOUD']) && \class_exists('OC');
	}
	...
	public function Init() : void
	{
		if (static::IsOwnCloud()) {
			...
			if (\class_exists('OC_App')) {
				$sAppPath = \rtrim(\trim(\OC_App::getAppWebPath('f7mail')), '\\/').'/app/';
			}
			if (!$sAppPath) {
				$sUrl = \MailSo\Base\Http::SingletonInstance()->GetUrl();
				if ($sUrl && \preg_match('/\/index\.php\/apps\/f7mail/', $sUrl)) {
					$sAppPath = \preg_replace('/\/index\.php\/apps\/f7mail.+$/',
						'/apps/f7mail/app/', $sUrl);
				}
			}
	...
	public function DoAttachmentsActions(\f7mail\AttachmentsAction $data)
	{
...
/*
		...
		if (!static::IsOwnCloud() ||
			!isset($_ENV['___f7mail_owncloud_email']) ||
			!isset($_ENV['___f7mail_owncloud_password']) ||
			empty($_ENV['___f7mail_owncloud_email'])
		)
		{
		...
		$sEmail = $_ENV['___f7mail_owncloud_email'];
		$sPassword = $_ENV['___f7mail_owncloud_password'];
		...
		try
		{
			...
			$bLogout = !($oAccount instanceof \f7mail\Model\Account);
		}
