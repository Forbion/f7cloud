require('../assets/NcAppContent-Bb5hgGi3.css');
"use strict";
...
const instanceName = capabilities.getCapabilities().theming?.name ?? "Nextcloud";
const activeApp = initialState.loadState("core", "active-app", appName);
const localizedAppNameState = initialState.loadState("core", "apps", {});
const localizedAppName = (Array.isArray(localizedAppNameState) ? localizedAppNameState.find((app) => app.id === activeApp)?.name : localizedAppNameState[activeApp]?.name) ?? appName;
const _sfc_main = {
  name: "NcAppContent",
  components: {
  ...
  computed: {
    paneConfigID() {
      ...
      try {
        ...
        logger.logger.info("[NcAppContent]: falling back to global nextcloud pane config");
...
//# sourceMappingURL=NcAppContent-POt1Lj3W.cjs.map
